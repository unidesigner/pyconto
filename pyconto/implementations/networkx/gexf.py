...
networks structures, their associated data and dynamics. Started in 2007
at Gephi  project by different actors, deeply involved in graph exchange
issues, the gexf specifications are mature enough to claim being both
extensible and open, and suitable for real specific applications."

http://gexf.net/format/

Format
------
Gexf is an XML format.  See
http://gexf.net/format/schema.html for the specification and 
...
def write_gexf(G, path, encoding='utf-8'):
   """Write G in Gexf XML format to path
   ...
   G : graph
      A Gexf graph
   path : file or string
      File or filename to write (ending .gexf).  
   ...
   Notes
   -----
   This implementation does not support writing networkx graphs.
   """
   fh = _get_fh(path, mode='wb')
   header='<?xml version="1.0" encoding="%s"?>\n'%encoding
   fh.write(header.encode(encoding))
   fh.write(etree.tostring(G.getXML(),encoding='utf-8'))
   fh.close()
...
def read_gexf(path):
    ...
    Returns
    -------
    graph: Gexf graph container, possibly containing multiple graphs  

    """
    fh=_get_fh(path,mode='rb')
    return GexfImport(fh).gexf()
...
class Graph :
	...
	def addNodeAttribute(self,title,defaultValue,type="integer",mode="static", force_id="") :
		...
		if force_id == "" :
			...
			id=force_id
		#  modify Nodes with default
		self._nodesAttributes[id]={"title":title , "default":defaultValue, "mode":mode, "type":type}
	...
	def addEdgeAttribute(self,title,defaultValue,type="integer",mode="static", force_id=""):
		...
		if force_id == "" :
			...
			id=force_id
		# modify Nodes with default
		self._edgesAttributes[id]={"title":title, "default":defaultValue, "mode":mode, "type":type }
...
class GexfImport :
	# class coded by Elie Rotenberg, medialab 20/07/2010
